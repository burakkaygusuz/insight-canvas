...
export default function Dashboard() {
  ...
  // Inside the JSX, below the search input form:
  // Find where the form ends and insert the loading indicator
    ...
    <main className="bg-background text-foreground relative min-h-screen overflow-hidden transition-colors duration-300">
      ...
      <header className="bg-background/80 sticky top-0 z-50 w-full border-b border-white/10 backdrop-blur-md">
        <div className="container mx-auto flex h-16 items-center justify-between px-4">
          <button
            type="button"
            className="focus:ring-primary flex items-center gap-2 rounded-lg p-1 transition-opacity hover:opacity-80 focus:ring-2 focus:ring-offset-2 focus:outline-none"
            onClick={() => setAppState(config.apiKey ? 'upload' : 'connect')}
          >
          ...
          {appState !== 'connect' && <SettingsModal config={config} onSave={setConfig} />}
        </div>
      </header>

      <div className="container mx-auto flex min-h-[calc(100vh-4rem)] flex-col items-center justify-start px-4 py-12">
